...
[![NuGet](https://img.shields.io/nuget/v/Shiny.DocumentDb.Mcp.svg?label=MCP)](https://www.nuget.org/packages/Shiny.DocumentDb.Mcp/)
[![NuGet](https://img.shields.io/nuget/v/ShinyDocDbMcp.svg?label=MCP+Tool)](https://www.nuget.org/packages/ShinyDocDbMcp/)
[![NuGet](https://img.shields.io/nuget/v/Shiny.DocumentDb.AspNetCore.svg?label=REST+Endpoints)](https://www.nuget.org/packages/Shiny.DocumentDb.AspNetCore/)
[![NuGet](https://img.shields.io/nuget/v/Shiny.DocumentDb.Extensions.VectorData.svg?label=VectorData)](https://www.nuget.org/packages/Shiny.DocumentDb.Extensions.VectorData/)
...
- **Single-row terminals** — `First()` / `FirstOrDefault()` / `Single()` / `SingleOrDefault()`, with predicate (`First(u => u.Age == 40)`) and string-filter (`First("status == 'open'")`) overloads. The row limit reaches the provider — relational stores emit `LIMIT 1`, MongoDB/Cosmos page server-side — so a single-document read never materializes the whole match set. `Single` fetches two rows so "more than one matched" costs no extra round trip.
- **Raw JSON results from the typed query** — end a `Query<T>()` with JSON instead of `T`, so a document read only to be written back out never becomes an object. `store.Query<Order>().Where(o => o.Id == id).FirstOrDefaultRawJson()` hands back the stored body unparsed for `Results.Content(raw, "application/json")`, and `.WriteJsonArrayTo(ctx.Response.Body, ct)` streams the whole match set into an HTTP response as one array — never buffered, never re-serialized. A node lane returns `JsonObject`s instead (`ToJsonList`, `ToJsonAsyncEnumerable`, `FirstJson`/`FirstOrDefaultJson`, `SingleJson`/`SingleOrDefaultJson`, `ToJsonCursorPage`), all over the `RawJsonRows(maxRows)` primitive. The full typed builder still applies — `Where`, `OrderBy`, `Paginate`, global query filters, soft delete, tenancy — and the single-row terminals still push their row limit down. Available on **every** provider: the relational stores and Cosmos DB hand back the persisted body untouched, while the rest must materialize `T` to finish the query and re-serialize through the type's `JsonTypeInfo` (same JSON, same API, no saving). Types with encrypted properties throw — the stored body is ciphertext and only the typed terminals decrypt.
...
- **Unique indexes over JSON values (every provider but DuckDB)** — `cfg.MapUniqueIndex(x => x.Email)` makes a value unique per document type; composite keys are an anonymous type (`x => new { x.Region, x.AccountNumber }`), nested properties work, and `cfg.MapProperty(x => x.Sku, p => p.Unique())` is the single-property shorthand. A `filter:` constrains only the documents it matches — `filter: x => !x.IsDeleted` is the natural pairing with soft delete — and is re-evaluated on every write. A violating Insert/Update/Upsert/SetProperty/`ExecuteUpdate`/batch/session write stores nothing and throws `UniqueConstraintException` (type, index, properties and document id — never the duplicated value, which is often personal data). The rules are identical everywhere: scoped to the type (other types in a shared table may reuse the value) and, with shared-table multi-tenancy, to the tenant; a null or missing key part is unconstrained; values compare exactly, so normalize first for case-insensitive uniqueness. Relational providers (except DuckDB, which can't index a JSON expression and rejects the mapping at startup) and MongoDB/Amazon DocumentDB create a native unique index when the table/collection is initialized (partial where the engine has one, a NULL-folded or hashed key where it doesn't); Cosmos DB, LiteDB, IndexedDB, DynamoDB, Azure Table, Firestore, Redis and RavenDB keep their own index entries in step with each write.
...
- **Full AOT/trimming support** — every API has an optional `JsonTypeInfo<T>` parameter for source-generated JSON serialization. No reflection required. Configure a `JsonSerializerContext` once and all methods auto-resolve type info — no per-call `JsonTypeInfo<T>` needed. Set `UseReflectionFallback = false` to catch missing type registrations with clear exceptions instead of opaque AOT failures. Every shipping package builds trim/AOT **warning-free** and is marked `IsAotCompatible`, so a consumer's trimmer actually trims into it; the mapping APIs (`cfg.MapVersionProperty`, `cfg.MapSpatialProperty`, `cfg.MapVectorProperty`, `cfg.MapFullTextProperty`, `cfg.MapComputedProperty`, `cfg.MapBlob`) annotate their `T` with `[DynamicallyAccessedMembers]` so the properties they resolve by name survive. This is verified on every build by `samples/Sample.Aot`, which publishes with `PublishAot=true` and treats any trim/AOT warning as an error — not just static analysis, a real ILC run. Two permanent limitations are documented: grouped projections need a **named** type with a `JsonTypeInfo` (anonymous types cannot have one), and string-query parameters should use `IDictionary<string, object?>` rather than an anonymous bag.
...
- **JSON Merge Patch (Upsert)** — `store.Upsert(patch)` deep-merges a partial object into an existing document using SQLite's `json_patch()` (RFC 7396). The Id comes from the object. Only patched fields are overwritten; unset nullable fields are preserved. The out-of-body **sidecars follow the same rule** — a patch that omits a mapped blob, geometry or embedding leaves the stored one and its sidecar row untouched, so a partial patch never re-sends a payload and never drops the document out of spatial/vector search. The corollary: a merge can't *clear* one (a `null` is stripped from the patch) — use a write that carries the whole document, `Update` or `Upsert(doc, patchIfUpdate: false)`. (A mapped vector is a non-nullable `ReadOnlyMemory<float>` that serializes as `[]` when unset, so the merge drops the empty array too — otherwise a partial patch would silently overwrite the stored embedding.)
...
- **Multi-property set-based updates** — `Query<Order>().Where(...).ExecuteUpdate(b => b.Set(o => o.Status, "expired").Set(o => o.ClosedAt, now))` writes every assignment in **one** statement: relational providers nest the dialect's JSON-set expression, MongoDB combines them into one `$set`, Cosmos applies them in the same read-modify-write pass. One predicate evaluation, atomic on its own.
- **Joins across document types (relational providers + MongoDB)** — `store.Query<Order>().Join<Customer>((o, c) => o.CustomerId == c.Id, JoinKind.Left).Where((o, c) => c.Region == "eu" && o.Total > c.CreditLimit).OrderBy((o, c) => c.Name).Paginate(0, 50).Select((o, c) => new { o.Id, Customer = c == null ? null : c.Name }).ToList()` runs as one engine query: inner or left, `Where` over either side or across both, ordering and paging in the engine, and `Select` into any shape over the materialized documents (so anonymous types work and encrypted properties read as plaintext). `Where` clauses on the source query filter the left side; each side keeps its own global query filters and tenant scope, and the right side's go in the join condition so a soft-deleted match leaves a left join's row in place. The string form is `Join<Customer>("o", "c", "o.customerId = c.id").Where("c.region = 'eu'").Project("o.id, c.name as customer")` — every field alias-qualified (a LINQ join's parameter names are its aliases), and the comparison grammar accepts a field on the right-hand side everywhere. An encrypted property filters its own side but cannot be a join key; spatial, full-text, computed properties and `GroupBy` stay single-document. The relational providers emit one `INNER`/`LEFT JOIN` statement and MongoDB one `$lookup` aggregation; Amazon DocumentDB (no correlated `$lookup`), Cosmos DB, LiteDB, IndexedDB and the key-value stores throw `NotSupportedException`.
...
- **JSON collections (raw JSON, keyed by name or by type)** — one API for working in document JSON instead of CLR instances, addressed two ways. **`store.Collection("orders")`** is *schema-free*: no CLR type, no registered mappings, no migrations — the name becomes the row's `TypeName`, so it shares a table with your typed documents without either seeing the other, and needs zero schema change. **`store.Collection(typeof(Order))`** (or `store.Collection<Order>()`) is *late-bound*: a registered document type but no instance, for generic HTTP intake, message-bus payloads, ETL and gateways. Both expose `Insert` (a `JsonObject` returns the stored id; a `JsonArray` writes many atomically and returns the count), `Update`/`Upsert` with the same `patch` / `patchIfUpdate` flags, `Get`, `Remove`, `BatchRemove`, `Clear`, `CreateIndex`, a raw-SQL `Query(whereClause, parameters)` escape hatch, and a fluent string-grammar `Query()` returning `JsonObject`s with `Where`/`OrderBy`/`Paginate`/`Project`/`GroupBy`/`ToCursorPage`/`ExecuteUpdate`/`ExecuteDelete`. The body is stored **AS-IS**. A type-keyed collection rides the **full write pipeline** (tenancy, temporal history, versioning/CAS, spatial + vector + blob sidecars, interceptors, change notifications) and resolves field paths through the type's metadata, so it reaches the whole grammar including `hasflag`, the geo predicates and the Lucene functions. A name-keyed collection has none of those registrations — tenancy, transactions, paging, cursors and backup still apply — and instead **infers** each field's type as the query is built: from an explicit `total:number` hint, else from the other operand (`total > 100` is numeric), else from the function (`lower(name)` is a string), else `string`. A hint is required for `OrderBy`/`min`/`max`/`Project` over a numeric field, because an unhinted extract sorts lexicographically (`"100"` before `"9"`) on every provider except SQLite. Ids differ by target: schema-free generates a sortable **UUIDv7 string** into a configurable property (read case-insensitively, written verbatim, compared as a literal string), while type-keyed follows the type's own `IdKind` (`Guid` → v4, `int`/`long` → sequence, declared `string` → refuses to auto-generate). Collection names and field paths must be identifiers (`^[A-Za-z_][A-Za-z0-9_]{0,127}$`, max 16 dot-separated segments) because they reach SQL as literals — so keys with dashes, spaces or dots are storable but not addressable. **Relational providers only** (SQLite, SQLCipher, DuckDB, PostgreSQL, CockroachDB, SQL Server, MySQL, MariaDB, Oracle); the document-native and key-partitioned providers throw `NotSupportedException`, and collections are a store-level feature reached from a session via `session.Store.Collection(...)`.
...
- **Document metadata (`DocumentMetadata`, every provider)** — declare `public DocumentMetadata? Metadata { get; set; }` and the store fills in `CreatedAt`, `UpdatedAt` and `IsPersisted` from the timestamps it already keeps next to the body — plus `TenantId`, the owning tenant under shared-table multi-tenancy (`TenantIdAccessor`, relational providers; `null` otherwise, not queryable): on every read, and on the instance you pass to `Insert`/`Update`/`Upsert` after the write, with no re-read. No mapping call — the property is found by type and newed up when null, so a stored document never has a null `Metadata`. It is never written into the body (the envelope stays the only copy), and `Where`/`OrderBy` on `x.Metadata.CreatedAt`/`UpdatedAt` — or the string grammar's `Metadata.CreatedAt` — run against the envelope: pushed down on the relational providers, MongoDB, Amazon DocumentDB, Cosmos DB, Firestore, Azure Table and DynamoDB, evaluated in memory on Redis, RavenDB, LiteDB and IndexedDB. Temporal snapshots new it up unstamped (`IsPersisted == false`).
...
- **Spatial / geo queries** — point queries (`WithinRadius`, `WithinBoundingBox`, `NearestNeighbors`) **and full OGC geometry**. Map a `GeoPoint?` or a `Geometry?` property (`GeoLineString`, `GeoPolygon` with holes, `GeoMultiPoint`/`GeoMultiLineString`/`GeoMultiPolygon`, `GeoGeometryCollection`; GeoJSON-serialized) with `cfg.MapSpatialProperty(x => x.Area)`, then query with the `Geo`-prefixed topological predicate family — `GeoIntersects`, `GeoContainedBy`, `GeoContains`, `GeoDisjoint`, `GeoTouches`, `GeoCrosses`, `GeoOverlaps`, `GeoEquals`, `GeoCovers`, `GeoCoveredBy` — plus `GeoWithinDistance(geometry, meters)`. Every predicate takes an optional `orderByDistanceFrom` (point or geometry) and returns `SpatialResult<T>` with `DistanceMeters`; `NearestNeighbors` works over geometry too. The `Geometry` model also exposes in-memory `Area`/`Length`/`Centroid`/`IsValid`/`MakeValid` (C# accessors — to filter by a measurement server-side, compute the scalar in your app and store it as a normal indexed field). **SQLite** (R\*Tree) and **PostgreSQL / MySQL / SQL Server / Oracle / DuckDB** (envelope-sidecar; PostgreSQL provisions PostGIS and DuckDB loads `spatial` unless you set `PortableSpatial = true`) run the full family via a bbox prune + in-process relate; **CosmosDB** pushes `ST_INTERSECTS`/`ST_WITHIN`/`ST_DISTANCE` down; **MongoDB** uses a `2dsphere` index (`$geoIntersects`/`$geoWithin`/`$near`). The mapped property can be nullable — documents with a `null` location are left out of the spatial index (no exception on write, and clearing a location on update purges its stale entry). One geometry is mapped per type, and it is the only one spatial queries can name: a second `Geometry?` property stores and round-trips fine, but naming it in a `Geo*` call, a `DocumentFunctions` predicate or the string grammar throws (most providers answer from the sidecar's geometry column, which holds only the mapped property). Several shapes that are *one* semantic slot belong in a single `GeoMultiPolygon`/`GeoGeometryCollection`, whose union envelope is the right index key.
...
- **Multi-tenancy, two models** — **shared-table** (`AddDocumentStore(configure, multiTenant: true)`) adds a `TenantId` column and filters every read and write, and **tenant-per-database** (`AddMultiTenantDocumentStore(factory)`) gives each tenant its own store. Both identify the caller through a user-implemented `ITenantResolver` (register it *scoped* and it resolves from the request's own DI scope), and consumer code is unchanged either way. Tenant-per-database takes an options factory (relational — the store is built for you and tagged `db.namespace` = tenant) or a **built store**, so tenants can live on any provider. Stores are built on first touch and held in a **bounded** cache — `MaxCachedStores` LRU + `IdleTimeout`, with a lease per DI scope so eviction never closes a store a request is still using — and `TenantStoreOptions.SeedFromRegisteredSeeders()` runs your registered seeders per tenant (versioned run-once, per tenant database) instead of once at startup. `ITenantStoreManager` adds `ActiveTenants` / `WarmAsync` / `EvictAsync` for onboarding and offboarding without a restart.
...
- **Transactional outbox** — `o.AddOutbox()` + `services.AddDocumentOutbox<TDispatcher>()` records a domain event in the *same transaction* as the write that caused it, then delivers it in the background. `session.Add(order).Enqueue(new OrderPlaced(...))` commits both rows together or neither, and `cfg.PublishToOutbox(ctx => ...)` does it declaratively per type. Delivery is at-least-once with an attempt counter, exponential backoff and dead-lettering; claiming is per-message optimistic concurrency, so any number of workers scale by just running. `store.WatchOutbox(...)` is a read-only stream for dashboards and health checks, `IOutboxAdmin` is the in-process operational view (alert on the *age* of the oldest pending message, not the depth), and the `traceparent` captured at enqueue is restored at dispatch. Requires a store whose unit of work is a real transaction — relational + LiteDB; every other backend compensates rather than commits, so a new `IDocumentStore.SupportsTransactions` capability fails the host start by name instead of shipping a dual write dressed as an outbox.
- **Field-level encryption** — `cfg.MapProperty(x => x.Ssn, p => p.Encrypt())` stores the property as AES-256-GCM ciphertext (`enc:1:<keyId>:<base64>`) on **every** provider and returns plaintext on read. Installed as a `JsonTypeInfo` modifier rather than an interceptor, so every write path — including temporal history and backup export — is covered by construction and no provider knows about it. `EncryptionMode.Deterministic` keeps equality filters working by rewriting the predicate's constant into ciphertext (and leaks equality/frequency — opt in per property); anything that cannot be answered against ciphertext throws with an explanation instead of matching nothing. Key rotation is a key ring plus `store.RewrapAsync<T>()`, values written before the mapping keep reading, and it is AOT-clean. In the core package — AES-GCM is in the BCL, so there is nothing extra to install. **Encryption is at rest**: a materialized document holds the value, so `Get`/`ToList`, OData responses and the AI tool results all return plaintext — mapping a property does not keep it out of an HTTP response (project to a DTO for that). Because the converters are symmetric, serializing a materialized document back through the store's own options would re-encrypt it; `DocumentEncryption.PlaintextView(options)` is the writer for that, and returns the same instance when nothing is encrypted. `DocumentEncryptionFormat` is the public read-only contract for the envelope itself — `TryParse` gives you the version and key id without any key — so a backup checker, a repair job or the Admin UI can tell an envelope from plaintext instead of hand-rolling a `StartsWith("enc:")` test that mis-classifies an ordinary value.
- **JSON Schema validation (`Shiny.DocumentDb.JsonSchema`)** — attach a JSON Schema (draft 2020-12, via [JsonSchema.Net](https://www.nuget.org/packages/JsonSchema.Net)) to a document type and the store validates the exact JSON about to be persisted just before the write. **No DI required** — `options.ConfigureDocument<Customer>(cfg => cfg.MapJsonSchema(schemaJson))` needs no container (repeated calls accumulate into one interceptor), so it works with a hand-built `new DocumentStore(options)`; the DI form is `services.AddDocumentJsonSchema(o => o.MapJsonSchema<Customer>(schemaJson))`. A failure throws `DocumentSchemaValidationException` with field-level errors and rolls the write back. Map schemas by `JsonSchema` object, JSON text, `Stream`, or `cfg.MapJsonSchemaFromFile(path)`. Enforces what the C# type can't — `maxLength`, ranges, `pattern`, `enum`, `additionalProperties:false`, reference-type required-ness — and asserts `format` (email/uuid/date-time) by default. Schema names match the **serialized** (camelCase) JSON names. Runs on `BeforeWrite`, so it composes with `Shiny.DocumentDb.AppDataSync` for free (invalid documents never reach the outbox); suppressed writes (inbound sync, bulk import) skip validation by design.
...
- **.NET Aspire integration (`Shiny.DocumentDb.Aspire.Hosting` / `.Client` / `.Orleans`)** — make the backend a deployment decision: `builder.AddPostgresDocumentStore("orders").WithSeeder(...)` in the AppHost picks the provider (Postgres/SQL Server/MySQL/SQLite) and gates seeding to two triggers — the first time that database is set up, or a destructive recreation (`DocumentStoreSeedMode.Recreate`), with the "already seeded" marker kept inside the database so wiping the data re-arms it; the consuming service calls `builder.AddDocumentStore("orders")` to get the keyed store wired with health checks + OpenTelemetry. The client opens up to DI-aware setup — `configureServiceOptions: (sp, o) => …` configures options with the resolved `IServiceProvider`, and a `MultiTenant` settings flag registers a shared-table multi-tenant store from a registered `ITenantResolver` in one line. A source-generated typed `DocumentContext` can be backed by an Aspire resource too — `builder.Services.AddOrdersContext(builder.AddDocumentContextProvider("orders"))`. `.WithAi(provider, model, key)` on the admin resources configures the admin tool's AI assistant from the AppHost (read-only in the tool once the host owns it; `WithAiWrites` is a separate, deliberate grant), and the terminal tool takes the same as `--ai-provider`/`--ai-model` flags. `silo.UseAspireDocumentDb("orders")` backs Orleans grain storage/reminders/clustering/directory with the same Aspire-provisioned store, and `DocumentDbOrleansFeatures.Streams` adds the persistent stream provider (opt-in, since streams need a backend with row-level locking). Server-tier only.
- **Admin UI (`ShinyDocDbMyAdmin`, `ShinyDocDbMyAdmin.Tui`)** — a phpMyAdmin-style front end for the relational stores, as a container image (`ghcr.io/shinyorg/shiny-docdb-myadmin`, mirrored to `aritchie/shiny-docdb-myadmin` on Docker Hub), as a Docker Desktop extension (`docker extension install aritchie/shiny-docdb-myadmin-extension`) that starts it and wires up the database containers already running on your machine, and as a terminal app (`dotnet tool install -g ShinyDocDbMyAdmin.Tui`, then `shinydocdb`) that share one admin layer, one connection store and one bundle format. Browse and edit documents in a paged, filterable grid, run SQL in the target dialect with bound parameters, inspect the inferred shape of a type and create/drop JSON property indexes, import/export JSON/NDJSON/CSV, view GeoJSON geometry on a rendered map or blob sidecar payloads with preview and download, read field-level-encryption envelopes and report key coverage across a type without holding any key (and refuse to overwrite one with clear text), triage the outbox queue and dead letters, and ask an AI assistant about the data — read-only by default, with per-connection opt-ins to allow the assistant to `insert_document`, `update_document` and `delete_document` on that connection (OpenAI / Azure OpenAI / Anthropic / any OpenAI-compatible endpoint, configured per connection and removable outright for demo deployments). It drops into an AppHost in one line — `builder.AddDocumentDbAdmin(port: 8085).WithReference(store)` — and every store you reference shows up already connected. On Aspire 13.5+ the terminal front end models the same way and needs no container: `builder.AddDocumentDbAdminTerminal().WithReference(store).WithStartupProfile(store)`, attached to with `aspire terminal attach documentdb-terminal`. It reaches a file-backed store without a bind mount, and it is dev-loop only — terminal sessions don't deploy, so anything you publish still wants the container. See [Admin UI](#admin-ui).
- **`Microsoft.Extensions.VectorData` connector** — `Shiny.DocumentDb.Extensions.VectorData` puts MEVD's `VectorStore` / `VectorStoreCollection<TKey, TRecord>` face on any vector-capable provider, so the store drops into MEAI, the Microsoft Agent Framework, or Semantic Kernel wherever they expect that abstraction. Decorate the record with `[VectorStoreKey]`/`[VectorStoreVector]`, call `o.MapVectorRecord<Note>()` while configuring the options (which emits the equivalent `cfg.MapVectorProperty`), and `services.AddDocumentDbVectorStore(...)` registers one store behind both `IDocumentStore` and `VectorStore` — a single connection pool. Every other MEVD connector is single-store; this one runs the **same record model over many backends** (SQLite for dev/mobile, PostgreSQL/pgvector or SQL Server for prod, Cosmos/Atlas/Redis), swapped by configuration. MEVD's filter is already an `Expression<Func<T, bool>>`, so it reaches `NearestVectors` untouched and is still pushed into the ANN search where the provider supports it. Text queries embed through an `IEmbeddingGenerator` from the definition, the constructor, or DI. Vector-capable providers only — the rest throw `NotSupportedException` at construction; dynamic (`Dictionary<string, object?>`) collections and multi-vector records are not supported.
- **AI tool integration** — `Shiny.DocumentDb.Extensions.AI` exposes `IDocumentStore` operations as `Microsoft.Extensions.AI` tool functions for LLM agents. Register document types with per-type capability flags (`ReadOnly`, `All`, or individual operations), structured filter expressions with boolean combinators, field visibility control (`AllowProperties`/`IgnoreProperties`), non-removable row-level scope filters (`Where(...)`) the LLM cannot see or bypass — enforced on every tool including get/insert/update/delete — and page size caps. **Request-resolved scopes (13.0)** — `.Where<ITenantContext>((tenant, _) => o => o.TenantId == tenant.TenantId)` (plus raw-context and async forms) resolve the predicate per tool call from `AIFunctionArguments.Services`, so a shared server scopes per caller rather than per registration; it fails **closed** (a throwing filter, an unresolvable service or a call with no services fails the tool call rather than running unscoped) and a missing `Where<TService>` registration is a startup error. No DI required — call `store.CreateAITools(b => b.AddType(...))` on a hand-built `IDocumentStore` to get a `DocumentStoreAITools` directly; the DI form (`services.AddDocumentStoreAITools(...)` → resolve `DocumentStoreAITools`) builds the same thing from the container. Pass `.Tools` to any `IChatClient`.
- **MCP server (`Shiny.DocumentDb.Mcp` + the `ShinyDocDbMcp` dotnet tool)** — point Claude Code, Claude Desktop, Copilot or any Model-Context-Protocol client at a store and let it explore and query the data. The tools are the **same** `Extensions.AI` tools (one implementation, one security model, one set of tests), wrapped in the MCP envelope and joined by resources (`documentdb://types`, `.../{name}/schema`, `.../{name}/sample`, `documentdb://stats` — the data-touching ones run *through* the registered tools, so the scope and page caps apply to a resource read too), two prompts (`explain-collection`, `build-filter`), and an audit line per call (tool, caller, argument digest, outcome, duration). Read-only by default with **two** locks on the destructive path (the per-type capability *and* a server-wide `AllowWrites()`), page caps, property hiding, no raw-SQL tool and no schema mutation. The library hosts on Streamable HTTP in any ASP.NET app (`AddDocumentDbMcpServer(...)`.`WithHttpTransport()` + `app.MapDocumentDbMcp("/mcp")`); the stdio tool needs no compiled document classes — it discovers what to expose from the stored `TypeName` discriminators and takes its connections from the admin tool's existing encrypted profile store.
- **REST + live-query endpoints (`Shiny.DocumentDb.AspNetCore`)** — `app.MapDocuments<Order>("/orders", …)` turns a document type into a complete HTTP resource: list, by-id, count, create, replace, RFC 7396 merge-patch, delete, and a live Server-Sent-Events tail at `/orders/stream`. Plain JSON, framework reference only, AOT-clean (routes are mapped as `RequestDelegate`s, so nothing reflects over a handler signature). Filtering uses the store's own string grammar behind a per-endpoint field allowlist (`AllowFilterOn`) — an unlisted field is a `400`, not a table scan — with `skip`/`take` (clamped to `MaxPageSize`), cursor paging, sparse fieldsets (`?fields=`), `ETag`/`If-Match` concurrency (`412`, or `428` with `RequireIfMatch`), and `ProblemDetails` throughout. `Scope(...)` is the HTTP twin of the AI tools' non-removable `Where`: resolved per request from the request's own DI scope, AND-ed, unremovable, and an out-of-scope document is `404` rather than `403` on every verb. List reads stream stored bodies straight to the socket through the raw-JSON terminals. `MapDocumentCollection` does the same for a schema-free JSON collection. The other half of the HTTP story from the OData packages — never map both on one prefix.
- **A public host surface (`Shiny.DocumentDb.Hosting`)** — the three things a package needs to host DocumentDb over a transport, so a host can live in any repo without an `InternalsVisibleTo` grant here first. `DocumentPredicate.Compile<T>(expr)` turns a predicate into a delegate **without `Reflection.Emit`** — a host has to check a scope against an *incoming* document on POST/PUT, before there is anything to query against, and `Expression.Compile()` would forfeit the trimmed/AOT guarantee of the hosting app; it is also the same evaluator the query layer uses, so the pushed-down predicate and the in-memory check cannot disagree about what a scope means. `DocumentFilter.Parse<T>` / `ParseJson` lower the string grammar to an expression, so a live tail can apply `?filter=` to changes arriving in memory. `DocumentStoreAccessor.GetMappings` / `GetVersionMapping` read the store's mapping registry on any provider, which is what an `ETag`/`If-Match` implementation needs. The AOT promise is enforced by a Native AOT publish and the signatures by a public-API surface test.
- **Orleans persistence stack (`Shiny.DocumentDb.Orleans`)** — a full Microsoft Orleans stack — **grain storage** (+ `PubSubStore`), **reminders** (`IReminderTable`), **cluster membership/clustering** (`IMembershipTable`), and **grain directory** (`IGrainDirectory`) — built entirely on `IDocumentStore`, so one set of implementations runs on every backend. `siloBuilder.AddDocumentDbGrainStorage(...)` / `.AddDocumentDbReminders(...)` / `.AddDocumentDbClustering(...)` / `.AddDocumentDbGrainDirectory("Default", ...)`. The Orleans ETag maps to a version-checked document (atomic CAS → `InconsistentStateException` on conflict), and grain state is stored as nested, **structured JSON** — so you can **query grain state directly without activating the grains** (reporting/dashboards/admin over the persisted read model, something Orleans' point-key storage contract can't do) and opt into `cfg.MapTemporal` for a free state-history audit trail. Companion packages `Shiny.DocumentDb.Orleans.MongoDb` / `Shiny.DocumentDb.Orleans.CosmosDb` wire grain storage for those backends in one call; a `StoreFactory` escape hatch covers the rest. (Membership needs multi-document transactions — relational or MongoDB replica set, not Cosmos.)
- **Orleans persistent streams (`siloBuilder.AddDocumentDbStreams("Default", …)`)** — durable, inspectable Orleans streams on the same store, so a cluster needs no queue service. Reads take no locks and write nothing per message (Orleans guarantees one pulling agent per queue, so the receiver works from a cursor), an adaptive backoff keeps an idle cluster off the database, and where the backend has a native change feed one subscription per silo wakes every receiver the moment anything is enqueued — including from another silo. Sequencing uses a per-queue counter row reserved under a row lock inside the enqueue transaction, **not** a native identity column: identity values are assigned at insert but rows appear at commit, so a late-committing transaction can be stepped over by the receiver's watermark and its event silently never delivered. Backends are gated by the new `SupportsPessimisticLocking` capability — PostgreSQL, SQL Server, MySQL, MariaDB, Oracle, CockroachDB — and the silo refuses to start on anything else. **Durable rewind** (`IsRewindable`): a subscriber can resume from a `StreamSequenceToken` older than anything in memory because the cache replays the events table — something no queue-backed provider can do — bounded by `Retention` and served by a second `(QueueId, StreamId, Seq)` index. `IStreamAdmin` (keyed by provider name) plus a **Streams** screen in ShinyDocDbMyAdmin report per-queue depth, lag, retained history and stuck streams; both are read-only, because a stream event is a position in a gap-free sequence rather than a unit of work you can requeue. This is a database-backed queue (thousands of events/sec on PostgreSQL), not an Event Hub replacement.
...
# Microsoft.Extensions.VectorData connector — use the store as a MEVD VectorStore
...
# GPS geofencing over document regions (iOS / Android / Mac Catalyst)
...
| `TableName` | `string` | `"documents"` | Name of the default shared document table. Types that do not set `cfg.Table` are stored here |
...
options.ConfigureDocument<User>(cfg => cfg.Table = cfg.TypeName);   // table named after the type
...
A custom table is **exclusive to one document type** — pointing a second type at the same name throws, because
`cfg.Table` means "give this type a place of its own". To put several types in the *same* table, leave
`cfg.Table` unset on all of them and set `TableName`; every type lands there, discriminated by `TypeName`.

### Custom Id property

Document types can use an alternate property as the document Id instead of the default `Id`. The Id property must be `Guid`, `int`, `long`, or `string`. Custom Ids can be combined with `cfg.Table`, or used on their own with `cfg.MapIdProperty` to keep the type in the default shared table.
...
options.ConfigureDocument<Customer>(cfg => { cfg.Table = "customers"; cfg.MapIdProperty(c => c.CustomerId); });
options.ConfigureDocument<Sensor>(cfg => { cfg.Table = "sensors"; cfg.MapIdProperty(s => s.DeviceKey); });
// Default shared table + custom Id
options.ConfigureDocument<BlogPost>(cfg => cfg.MapIdProperty(p => p.Slug));
...
| Member | Description |
|---|---|
| `cfg.Table = "orders"` | Explicit table name |
| `cfg.Table = cfg.TypeName` | Table named after the type |
| `cfg.MapIdProperty(x => x.MyKey)` | Custom Id property |
| `cfg.MapIdProperty("MyKey")` | AOT-safe string overload of the above |

- **Additive** — `ConfigureDocument<T>` can be called more than once for the same type
...
- **Id remapping is independent of table mapping** — use either, both, or neither
...
**Replacing a write instead of observing it (`ctx.Cancel()`)** — in `BeforeWrite`, `ctx.Cancel(bool succeeded = true)` tells the store to issue **no** write for that operation because the interceptor performed one itself: no `AfterWrite`, no change notification or temporal entry, and later interceptors are skipped. `Remove` returns the `succeeded` value. Set-based writes use `DocumentBulkContext.Cancel(int affected = 0)`, with `ctx.QueryAs<T>()` handing back the originating query (same predicate and filters; `null` for `Clear`, where `ctx.Store` is the handle) so it can be re-issued differently. Do the replacement write through `ctx.Store`/`ctx.Session` so it commits with the same unit. Throwing still aborts the write outright — cancelling is not an error path. This is what makes an interceptor able to change the *shape* of an operation (delete → update), and it is exactly how the built-in soft delete below is implemented.
...
await store.Get<Customer>("c1");                           // null — hidden by the filter
await store.Query<Customer>().IncludeDeleted().ToList();   // reads past it
await store.Query<Customer>().OnlyDeleted().ToList();      // just the flagged ones

await store.Restore<Customer>(x => x.Id == "c1");          // clear the flag (predicate, not id)
...
One flag per document type. `Restore` takes a predicate because a flagged document is invisible to a by-id write. `Insert`/`Upsert` stay unfiltered (as with any query filter), so a document inserted with the flag already set is immediately invisible. Because a set-based delete is re-issued as an update over the same query, spatial/vector/blob sidecar rows stay with the document instead of being orphaned. Change feeds emit `Updated` and temporal records an update — the write really is an update.

## Transactional Outbox

Records a domain event **in the same transaction as the write that caused it**, then delivers it in the background — no second datastore, no dual-write window. Two registrations, each doing one thing: `AddOutbox` maps the message type onto the store, `AddDocumentOutbox` starts the processor.
...
await session.SaveChanges();       // the order row and the message row commit together, or neither does
```

Always enqueue through the session doing the aggregate write — a `store.Insert` of the message in a separate call is a dual write with extra steps. Declaratively, per type, from inside the write's own transaction:
...
Delivery is **at-least-once** — consumers must be idempotent. `IOutboxDispatcher.Dispatch` throws to retry and returns to acknowledge, and is resolved from a fresh DI scope per message. Claiming is per-message optimistic concurrency: two workers that read the same message both write it back, and the loser gets a `ConcurrencyException` and moves on. No leases table, no leader election, and any number of workers scale by just running. Because the claim pushes `AvailableAt` forward by the backoff, the backoff doubles as a visibility timeout — a processor that dies mid-dispatch releases its message rather than stranding it. `OrderedPartitions` orders within a `PartitionKey`; a poison message blocks its own partition only, and only until it dead-letters.

```csharp
// Read-only observation — never claims, so it cannot compete with the processor.
await foreach (var m in store.WatchOutbox(o => o.States = OutboxStates.DeadLettered, ct))
    logger.LogError("Outbox dead letter {Id}: {Error}", m.Id, m.Error);

var admin = sp.GetRequiredService<IOutboxAdmin>();
await admin.PendingCount();
await admin.OldestPendingAt();     // ALERT ON THIS, not on the depth
await admin.Requeue(ids);          // dead letters only — scheduled and processed are refused
await admin.PurgeProcessed(DateTimeOffset.UtcNow.AddDays(-7));
```

A healthy busy system has a large pending count; a system whose processor died has an *old* one. Depth cannot tell those apart, age can — hence `OldestPendingAt` and the `db.client.outbox.pending` gauge beside it. `WatchOutbox` is a poll nudged early by change notification, not the other way round: a message becomes pending again because its `AvailableAt` elapsed, which is a clock event with no write behind it, and in-process notifications are blind to other writers anyway.

Telemetry comes with it: span `outbox.dispatch`, counters `db.client.outbox.dispatched` / `.dead_lettered`, a dispatch-duration histogram and a pending-depth gauge. The `traceparent` captured at enqueue is restored at dispatch, so a consumer's span links back to the request that caused the event.

**Provider support.** The outbox requires a store whose unit of work is a real transaction, checked once at host startup via `IDocumentStore.SupportsTransactions`:

| Tier | Providers |
|---|---|
| Supported | SQLite, SQLCipher, PostgreSQL, CockroachDB, SQL Server, MySQL, MariaDB, Oracle, DuckDB, LiteDB |
| Gated out | MongoDB, Cosmos DB, Redis, RavenDB, Azure Table, DynamoDB, Firestore, IndexedDB |

The gated backends implement a unit of work by *compensation* — a failed unit undoes its inserts, but a process that dies mid-unit runs no compensation at all, which is exactly the window an outbox exists to close. Cosmos DB is a further no: "same transaction" there means "same logical partition", and the store partitions by type name. `AddDocumentOutbox` throws at startup naming the provider rather than shipping a promise that does not hold; use `IChangeFeedDocumentStore` on those.

Messages are ordinary documents in their own table, so backup, replication and `ClearAll` carry them like anything else. Their wire names are pinned with `[JsonPropertyName]` (mirrored by `OutboxFields`) because other processes — the admin tool among them — read these rows. Without a generic host, `OutboxRunner` is the same work as a plain object: `DrainOnce()`, `PurgeExpired()`, `PendingDepth()`.
...
Opt a document type into append-only history with `cfg.MapTemporal`. Every mutation records a versioned snapshot to a per-type history sidecar, so you can read a document's state as of any point in time, audit changes, restore prior versions, and diff between versions. Opt-in per type — only mapped types pay the extra write.
...
History is an **optional capability**, not part of the universal CRUD contract — so it lives on its own interface, `ITemporalDocumentStore : IDocumentStore`, the same way observation lives on `IObservableDocumentStore` and the native change feed on `IChangeFeedDocumentStore`. Putting `History`/`AsOf`/`Restore`/… on `IDocumentStore` would force every consumer of a plain store to see seven methods that throw far more often than they work (they require the type to be `cfg.MapTemporal`-mapped), and force every backend to implement them. Asking for `ITemporalDocumentStore` instead makes "this store does history" a compile-time, discoverable fact. Resolve or cast to it:

```csharp
var store = serviceProvider.GetRequiredService<ITemporalDocumentStore>();
```

Calling a history method for a type that wasn't passed to `cfg.MapTemporal` throws `InvalidOperationException`.
...
> **IndexedDB:** because temporal adds new object stores, an existing database must be opened at a higher `Version` so the schema upgrade creates them — bump `options.Version` when adding `cfg.MapTemporal` to an already-deployed store. A fresh database needs no change.
...
`Shiny.DocumentDb.Extensions.VectorData` exposes the store as MEVD's `VectorStore` for the .NET AI ecosystem. Map each record while configuring the options — DocumentDb fixes its vector mappings when the store is built, so `GetCollection` only builds a facade:
...
`[VectorStoreKey]` must also be the document's id (`Id` by convention, otherwise `cfg.MapIdProperty`) and be `string`/`Guid`/`int`/`long`; one `[VectorStoreVector]` per record. `UpsertAsync` is a full replace, `IncludeVectors` defaults to `false`, and results are ordered nearest-first on every provider while the raw score keeps its backend-specific direction. `GetDynamicCollection` and hybrid keyword+vector search throw. For a store registered elsewhere — or on a provider with its own options class (Mongo/Cosmos/Redis) — use the no-argument `services.AddDocumentDbVectorStore()`, or `new DocumentDbVectorStore(store)` for the container-free path.
...
`ShinyDocDbMyAdmin` is a phpMyAdmin-style front end for DocumentDb stores. Connect to a
...
It comes in **two front ends over one admin layer**. The web one ships as a container image, on GHCR
and mirrored byte-for-byte to Docker Hub:

```bash
docker run -p 8085:8080 -v shiny-docdb-myadmin:/data ghcr.io/shinyorg/shiny-docdb-myadmin
# or: docker run -p 8085:8080 -v shiny-docdb-myadmin:/data aritchie/shiny-docdb-myadmin
```

On Docker Desktop there is a one-click way in — a **DocumentDb Admin** tab that starts that container,
runs the app inside the tab, and hands it every database container already on your machine, connected:
...
They share the connection store, the secret handling and the bundle format — `ShinyDocDbMyAdmin.Core`
holds the profiles, the document/schema/index/sidecar services and import/export — so a connection saved
in one is a connection the other opens, and a bundle written by either imports into the other. The
...
| **Encryption** | Appears when a type has [field-level encrypted](#field-level-encryption) properties. Reads the `enc:1:<keyId>:<base64>` envelope **with no key at all**: badges the value everywhere it would otherwise be a wall of base64, counts on demand how many values across the whole type sit under each key id (which is what tells you whether a `RewrapAsync<T>()` finished, and nothing else does — retiring a key early makes those documents unreadable), and reports mode only when the data proves it (a repeated ciphertext proves *deterministic*; nothing can prove *randomized*, so it is never claimed). Saving clear text over an envelope **throws** unless confirmed, because the library reads a non-envelope as pre-encryption plaintext and the failure is a silent loss of protection rather than an error; imports count the same thing and exports say plainly that they carry ciphertext. Query surfaces that cannot work are withdrawn (quick search) or explained (the filter console compiles to SQL, so it never encrypts your constant). Reading the values back is opt-in per connection, masked until revealed, never in an export, never in demo mode, and **never** in the assistant. |
