...
- **Data migrations** — register `IDocumentMigration`s to backfill or transform *existing* data at a version boundary (split a field, recompute a value, re-key documents). Each has a `long Version` (an EF-style `yyyyMMddHHmmss` timestamp), an `Up`, and an optional `Down`. Provider-agnostic (`IDocumentStore`-only) and tracked by a ledger of `DocumentMigrationRecord` documents — no system table. Pending migrations run once at startup **before seeders** (`AddDocumentMigration<T>()` / `AddDocumentMigration(version, name, up, down)`), or drive them via the injectable `IDocumentMigrator` (`GetCurrentVersionAsync`, `GetAppliedAsync`, `MigrateAsync`, `RollbackToAsync`) or `DocumentMigrationRunner.RunAsync` on MAUI.
