...
- **Spatial / geo queries** — `WithinRadius`, `WithinBoundingBox`, and `NearestNeighbors` methods with `GeoPoint` support. SQLite uses R*Tree virtual tables; CosmosDB uses native `ST_DISTANCE`/`ST_WITHIN`. Configure with `MapSpatialProperty<T>(x => x.Location)`. The mapped property can be nullable (`GeoPoint?`) — documents with a `null` location are simply left out of the spatial index (no exception on write, and clearing a location on update purges its stale index entry).
