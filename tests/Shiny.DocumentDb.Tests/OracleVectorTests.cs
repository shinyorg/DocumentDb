...
[Collection("Oracle")]
public class OracleVectorTests
{
    readonly OracleDatabaseFixture fx;

    public OracleVectorTests(OracleDatabaseFixture fx) => this.fx = fx;

    [Fact]
    public async Task SupportsVector_IsTrue()
    {
        var store = this.fx.CreateVectorStore("ora_vec_supports");
    ...
    {
        var store = this.fx.CreateVectorStore("ora_vec_topk");
        ...
        // Cosine distance: identical vectors → ~0, monotonically increasing thereafter.
    ...
    {
        var store = this.fx.CreateVectorStore("ora_vec_filter");
        foreach (var d in VectorTestSeed.Docs)
            await store.Insert(d);
    ...
    {
        var store = this.fx.CreateVectorStore("ora_vec_update");
        foreach (var d in VectorTestSeed.Docs)
            await store.Insert(d);
    ...
    {
        var store = this.fx.CreateVectorStore("ora_vec_remove");
        foreach (var d in VectorTestSeed.Docs)
            await store.Insert(d);
    ...
    {
        var store = this.fx.CreateVectorStore("ora_vec_l2", metric: VectorDistance.Euclidean);
        foreach (var d in VectorTestSeed.Docs)
            await store.Insert(d);
    ...
    {
        var store = this.fx.CreateVectorStore("ora_vec_dot", metric: VectorDistance.DotProduct);
        foreach (var d in VectorTestSeed.Docs)
            await store.Insert(d);
    ...
    {
        var store = this.fx.CreateVectorStore("ora_vec_dim");
