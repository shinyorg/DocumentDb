...
public class FirestoreBlobTests : IDisposable
{
    readonly FirestoreDatabaseFixture db;
    readonly string collection;
    readonly FirestoreDocumentStore store;

    public FirestoreBlobTests(FirestoreDatabaseFixture db)
    {
        this.db = db;
    ...
    {
        var bytes = Payload("firestore blob payload");
    ...
    public async Task Sweep_removes_orphaned_blobs()
    {
        ...
        using (var noBlob = this.db.CreateConfiguredStore(this.collection, _ => { }))
