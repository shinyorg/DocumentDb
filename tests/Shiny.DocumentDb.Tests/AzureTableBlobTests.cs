...
public class AzureTableBlobTests : IDisposable
{
    readonly AzureTableDatabaseFixture db;
    readonly string tableName;
    readonly AzureTableDocumentStore store;

    public AzureTableBlobTests(AzureTableDatabaseFixture db)
    {
        this.db = db;
    ...
    {
        var bytes = Payload("azure table blob");
    ...
    public async Task Sweep_removes_orphaned_blobs()
    {
        ...
        using (var noBlob = this.db.CreateConfiguredStore(this.tableName, _ => { }))
