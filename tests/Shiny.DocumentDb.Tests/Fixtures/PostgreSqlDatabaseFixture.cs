...
public class PostgreSqlDatabaseFixture : IDatabaseFixture, IDocumentStoreFixture, ITenantDocumentStoreFixture, ITemporalDocumentStoreFixture, ISpatialDocumentStoreFixture, IAsyncLifetime
{
    ...
    /// <summary>
    /// A pgvector store whose mappings the caller supplies — for suites that bring their own record types
    /// instead of the shared <see cref="VectorDoc"/>.
    /// </summary>
    public IDocumentStore CreateVectorStore(string tableName, Action<IDocumentStoreOptions> configure)
    {
        var opts = new DocumentStoreOptions
        {
            DatabaseProvider = this.CreateProvider(),
