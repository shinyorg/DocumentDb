...
public abstract class DocumentProviderBase
{
    ...
    /// <summary>Builds a write context (null when no per-doc interceptors are registered).</summary>
    protected DocumentWriteContext? NewWriteContext<T>(DocumentOperation op, string typeName, object? id, T? document) where T : class
        => this.Interceptors.NewWrite(op, typeName, id, document);
    ...
    protected Task<DocumentWriteContext[]?> RunBeforeWriteBatchAsync<T>(IList<T> documents, string typeName, CancellationToken ct) where T : class
        => this.Interceptors.BeforeWriteBatch(documents, typeName, ct);
