...
/// <summary>
/// Client-side <see cref="IDocumentQuery{T}"/> for RavenDB. Because the document body is stored as opaque JSON
/// (see <see cref="RavenDbDocument.DataJson"/>), an immediately-consistent id-prefix stream pulls every
/// document of the type, then predicates, ordering, pagination, projection, and aggregates run in memory via
/// <see cref="ExpressionInterpreter"/> (the LiteDB/DynamoDB model). Loading the whole type is a full scan —
/// map hot query paths accordingly.
/// </summary>
public class RavenDbDocumentQuery<T> : IDocumentQuery<T> where T : class
{
    ...
    internal RavenDbDocumentStore Store => this.store;
    ...
    RavenDbDocumentQuery(RavenDbDocumentQuery<T> source)
    {
    ...
    {
        var clone = new RavenDbDocumentQuery<T>(this);
    ...
    public IGroupedDocumentQuery<T, object> GroupBy(string keyField)
        => throw new NotSupportedException(
            "String GroupBy(\"field\") is not supported on the RavenDB provider. Use the typed GroupBy(keySelector).Select(g => …) form, or a relational store for the string grammar.");
    ...
    internal async Task<IEnumerable<T>> MaterializeForGroupingAsync()
    {
        ...
        await foreach (var doc in this.store.LoadDocumentsAsync(this.typeInfo, CancellationToken.None).ConfigureAwait(false))
            items.Add(doc);
    ...
    public IDocumentQuery<TResult> Select<TResult>(
        ...
        => new RavenDbProjectedDocumentQuery<T, TResult>(this, selector);
    ...
    public DocumentQueryString ToQueryString()
    {
        ...
        foreach (var predicate in this.GetEffectivePredicateExpressions())
            where += $" and ({RavenExpressionVisitor.Translate(predicate, this.store.JsonOptions, this.typeInfo)})";

        var rql = $"from \"RavenDbDocuments\" where {where}";
        if (this.orderBys.Count > 0)
        {
            var terms = this.orderBys.Select(o =>
            {
                var field = ResolveOrderField(o.Selector);
    ...
    internal async Task<IEnumerable<T>> MaterializeAsync(CancellationToken ct)
    {
        ...
        {
            var typedFunc = ExpressionInterpreter.Interpret<T, object>(selector);
            if (ordered == null)
...
internal class RavenDbProjectedDocumentQuery<TSource, TResult> : IDocumentQuery<TResult>
    where TSource : class
    where TResult : class
{
    readonly RavenDbDocumentQuery<TSource> source;
    readonly Func<TSource, TResult> compiledSelector;

    internal RavenDbProjectedDocumentQuery(RavenDbDocumentQuery<TSource> source, Expression<Func<TSource, TResult>> selector)
    {
        this.source = source;
