...
/// <summary>
/// Client-side <see cref="IDocumentQuery{T}"/> for Azure Table Storage. A single-partition native query
...
/// </summary>
public class AzureTableDocumentQuery<T> : IDocumentQuery<T> where T : class
{
    ...
    AzureTableDocumentQuery(AzureTableDocumentQuery<T> source)
    {
    ...
    {
        var clone = new AzureTableDocumentQuery<T>(this);
    ...
    public DocumentQueryString ToQueryString()
    {
        ...
        var pushdown = this.store.BuildPushdownFilter(this.GetEffectivePredicateExpressions());
...
internal class AzureTableProjectedDocumentQuery<TSource, TResult> : IDocumentQuery<TResult>
    where TSource : class
    where TResult : class
{
    readonly AzureTableDocumentQuery<TSource> source;
    readonly Func<TSource, TResult> compiledSelector;

    internal AzureTableProjectedDocumentQuery(AzureTableDocumentQuery<TSource> source, Expression<Func<TSource, TResult>> selector)
    {
        this.source = source;
