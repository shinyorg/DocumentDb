...
/// <summary>
/// The Azure Table Storage per-type mappings. Every type lives in one table there, so a type is separated by its
...
/// </summary>
public static class AzureTableDocumentTypeBuilderExtensions
{
    /// <summary>
    ...
    static AzureTableDocumentStoreOptions Require<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties)] T>(DocumentTypeBuilder<T> cfg) where T : class
    {
        ArgumentNullException.ThrowIfNull(cfg);
        return cfg.Options as AzureTableDocumentStoreOptions
            ?? throw new InvalidOperationException(
                $"This is a Azure Table Storage mapping, but the store is configured with '{cfg.Options.GetType().Name}'.");
    }
}
